...
//  ####################    SIMD Approach           #####################
    ...
    prev_res = cur_res;
    to_continue = true;
    abs_err = -1; // Just guard value
    rel_err = -1; // Just guard value
    ...
    while (to_continue) {
        ...
        steps *= 2;
        cur_res = simd_integrate(langermann_f, steps, int_args);
        abs_err = fabs(cur_res - prev_res);
        rel_err = fabs((cur_res - prev_res) / cur_res);
...
//  #####################   Program Output Block   ######################
    std::cout << "SIMD Result = " << cur_res << std::endl;
    std::cout << "SIMD Absolute_error = " << abs_err << std::endl;
    std::cout << "SIMD Relative_error = " << rel_err << std::endl;
    std::cout << "SIMD Time = " << to_us(time_to_calculate) << std::endl;
