...
//    off_t filesize;
    ...
    if (archive_read_open_memory(a, buffer.c_str(), buffer.size())) {
        std::cerr << "Error: Can not open archive_obj form memory!" << archive_error_string(a) << std::endl;
    ...
    while (status > ARCHIVE_FATAL) {
        ...
        if (status == ARCHIVE_EOF) {
            ...
            if (status < ARCHIVE_WARN) {
                std::cerr << "Error: Serious archive_obj damage!" << archive_error_string(a) << std::endl;
                continue;  // TODO: test for critical status
            }
            std::cerr << "Warning: Some bad blocks in the archive_obj!" << archive_error_string(a) << std::endl;
        }
