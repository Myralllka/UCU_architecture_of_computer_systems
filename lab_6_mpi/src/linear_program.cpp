...
void linear_program(m_matrix<double> matrix, const ConfigFileOpt &config) {
    ...
    double alpha = config.get_thermal_conduction() / config.get_density() / config.get_specific_heat_capacity();
    for (int i = 0; i < config.get_data_cycles(); ++i) {
        if (i % 2) {
            count_next_step(matrix, second_matrix, alpha, config);
        } else {
            count_next_step(second_matrix, matrix, alpha, config);
        }
    }
