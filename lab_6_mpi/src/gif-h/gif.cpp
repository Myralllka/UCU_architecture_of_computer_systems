...
bool GifWriteFrame( GifWriter* writer, unsigned char const* image, unsigned int width, unsigned int height, unsigned int delay, int bitDepth, bool dither) {
    if(!writer->f) return false;
...
void GifWriteLzwImage(_IO_FILE* f, unsigned char const* image, unsigned int left, unsigned int top,  unsigned int width, unsigned int height, unsigned int delay, GifPalette* pPal){
    ...
    auto* codetree = (GifLzwNode*)GIF_TEMP_MALLOC(sizeof(GifLzwNode)*4096);
...
void GifWritePalette( const GifPalette* pPal, _IO_FILE* f ){
...
void GifThresholdImage( unsigned char const* lastFrame, unsigned char const* nextFrame, unsigned char* outFrame, unsigned int width, unsigned int height, GifPalette* pPal ) {
...
void GifDitherImage( unsigned char const* lastFrame, unsigned char const* nextFrame, unsigned char* outFrame, unsigned int width, unsigned int height, GifPalette* pPal ) {
    int numPixels = (int)(width * height);

    // quantPixels initially holds color*256 for all pixels
    // The extra 8 bits of precision allow for sub-single-color error values
    // to be propagated
    auto *quantPixels = (int32_t *)GIF_TEMP_MALLOC(sizeof(int32_t) * (size_t)numPixels * 4);
...
void GifMakePalette( unsigned char const* lastFrame, unsigned char const* nextFrame, unsigned int width, unsigned int height, int bitDepth, bool buildForDither, GifPalette* pPal ) {
...
int GifPickChangedPixels( unsigned char const* lastFrame, unsigned char* frame, int numPixels ){
    int numChanged = 0;
